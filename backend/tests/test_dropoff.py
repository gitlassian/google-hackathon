"""viewers_remaining_pct: a plain fact, not a stand-in for Studio's metric.
...
from app.youtube.mapping import viewers_remaining_pct
...
def test_measures_who_is_left_after_the_swipe_decision():
    ...
    assert viewers_remaining_pct(ROWS, duration_sec=49) == pytest.approx(77.3, abs=0.1)


def test_a_longer_window_leaves_fewer_viewers():
    early = viewers_remaining_pct(ROWS, duration_sec=49, at_seconds=1.0)
    later = viewers_remaining_pct(ROWS, duration_sec=49, at_seconds=2.0)

    assert later < early


def test_needs_a_duration_to_know_where_one_second_falls():
    assert viewers_remaining_pct(ROWS, duration_sec=None) is None
    assert viewers_remaining_pct(ROWS, duration_sec=0) is None


def test_has_no_answer_without_the_counters():
    rows = [{"elapsedVideoTimeRatio": 0.01, "audienceWatchRatio": 1.6}]

    assert viewers_remaining_pct(rows, duration_sec=49) is None


def test_has_no_answer_without_rows():
    assert viewers_remaining_pct([], duration_sec=49) is None
...
def test_never_goes_negative_if_more_stopped_than_started():
    ...
    assert viewers_remaining_pct(rows, duration_sec=49) == 0.0
